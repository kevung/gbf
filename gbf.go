// Package gbf implements the Gammon Binary Format (GBF) v0.3 specification.
//
...
//
// The library uses github.com/kevung/bgfparser, github.com/kevung/gnubgparser,
// and github.com/kevung/xgparser to parse the different backgammon file formats.
package gbf
...
type BaseRecord struct {
	...
	// Positive values = Player X checkers, encoded in low bits.
	// The sign/player is determined by which layer bitboards are set.
	...
	// Zobrist is the deterministic hash of this position.
	Zobrist uint64
...
// It is inserted as a standalone record (with a zeroed base record) to
// delimit games within a GBF file.
type GameBoundary struct {
	GameNumber uint16
	ScoreX     uint16 // score at start of game (points won so far)
	ScoreO     uint16
	Winner     int8  // 0=player1, 1=player2, -1=unfinished
	PointsWon  uint8 // points won in this game
...
type PositionState struct {
	// Board stores checker counts per point (1-24).
	// Positive = Player X (player 1), Negative = Player O (player 2).

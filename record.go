...
func PositionToBaseRecord(pos *PositionState) (*BaseRecord, error) {
	...
	// Build layer bitboards and point counts
	for i := 0; i < 24; i++ {
		count := pos.Board[i]
		if count > 0 {
			// Player X checkers
			...
			// Player O checkers (stored as positive count)
			absCount := -count
...
func BaseRecordToPosition(rec *BaseRecord) *PositionState {
	...
	for i := 0; i < 24; i++ {
		...
		// Determine player from layer bitboards
...
// computePipCountX computes Player X's pip count from the position.
// Player X moves from high points to low: pip distance = point_index + 1.
...
// computePipCountO computes Player O's pip count from the position.
// Player O moves from low points to high: pip distance = 24 - point_index.
// Bar checkers count as 25 pips.
...
// validateCheckerCounts checks that each player has exactly 15 checkers total.
func validateCheckerCounts(pos *PositionState, rec *BaseRecord) error {
	...
	// Validate layer bitboards match counts
	for i := 0; i < 24; i++ {
		count := int(rec.PointCounts[i])
		for layer := 0; layer < 4; layer++ {
			var hasX, hasO bool
			if rec.LayersX[layer]&(1<<uint(i)) != 0 {
				hasX = true
			}
			if rec.LayersO[layer]&(1<<uint(i)) != 0 {
				hasO = true
			}
			if count > layer {
				if !hasX && !hasO {
...
func MarshalBaseRecord(rec *BaseRecord) []byte {
	...
	// Layer bitboards: X layers (4×4=16 bytes) then O layers (4×4=16 bytes)
	for i := 0; i < 4; i++ {
	...
	// Point counts: 24 × 4 bits = 12 bytes, packed contiguously
	for i := 0; i < 24; i += 2 {
...
func MarshalRecord(rec *Record) ([]byte, error) {
	// Set block count in the base record before marshaling
	...
	for _, block := range rec.Blocks {
		// Block header: type(1) + version(1) + length(2) = 4 bytes
...
func MarshalCheckerPlayAnalysis(cpa *CheckerPlayAnalysis) []byte {
	// 1 byte move_count + per move: 4+2+2+2+2+2+2+2+1+1+8 = 28 bytes
	...
	for i := 0; i < int(cpa.MoveCount); i++ {
		...
		offset++
		// MoveEncoding: 4 submoves × 2 bytes = 8 bytes
		for j := 0; j < 4; j++ {
...
func MarshalCubeDecisionAnalysis(cda *CubeDecisionAnalysis) []byte {
	// 6×uint16 + 5×int32 + 1×uint8 = 12 + 20 + 1 = 33 bytes
	buf := make([]byte, 33)
	offset := 0
...
// Strings are length-prefixed (uint8 length + UTF-8 bytes).
...
func NewCheckerPlayBlock(cpa *CheckerPlayAnalysis) AnalysisBlock {
	...
	return AnalysisBlock{
		BlockType:   BlockTypeCheckerPlay,
		Version:     1,
		BlockLength: uint16(len(payload)),
		Payload:     payload,
	}
}
...
func NewCubeDecisionBlock(cda *CubeDecisionAnalysis) AnalysisBlock {
	...
	return AnalysisBlock{
		BlockType:   BlockTypeCubeDecision,
		Version:     1,
		BlockLength: uint16(len(payload)),
		Payload:     payload,
	}
...
func NewEngineMetadataBlock(em *EngineMetadata) AnalysisBlock {
	...
	return AnalysisBlock{
		BlockType:   BlockTypeEngineMetadata,
		Version:     1,
		BlockLength: uint16(len(payload)),
		Payload:     payload,
	}
...
// Strings are length-prefixed (uint8 length + UTF-8 bytes),
// followed by uint16 MatchLength.
...
func NewMatchMetadataBlock(md *MatchMetadata) AnalysisBlock {
	...
	return AnalysisBlock{
		BlockType:   BlockTypeMatchMetadata,
		Version:     1,
		BlockLength: uint16(len(payload)),
		Payload:     payload,
	}
...
func NewGameBoundaryBlock(gb *GameBoundary) AnalysisBlock {
	...
	return AnalysisBlock{
		BlockType:   BlockTypeGameBoundary,
		Version:     1,
		BlockLength: uint16(len(payload)),
		Payload:     payload,
	}
